...
mint_jwt = _load_module("espn_integration.clerk_auth", _HERE / "clerk_auth.py").mint_jwt
...
def _cleanup_test_league(context, test_league_id: str) -> None:
    ...
    resp = requests.delete(
        ...
        params={"platform": "ESPN"},  # Platform enum is case-insensitive
...
def before_all(context):
    ...
    test_league_id = os.environ["TEST_ESPN_LEAGUE_ID"]
    ...
    # Resolve Clerk auth config used to delete the test league through the API, and
    # the private ESPN cookies (stored as SecureString SSM parameters, never in CI).
    ...
    context.espn_s2 = get_ssm_parameter(os.environ["ESPN_S2_SSM_PARAM"])
    context.espn_swid = get_ssm_parameter(os.environ["ESPN_SWID_SSM_PARAM"])
    context.espn_season = os.environ["TEST_ESPN_SEASON"]
