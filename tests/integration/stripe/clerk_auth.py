"""Mint a Clerk session JWT for the Stripe integration tests.
...
The Stripe billing endpoints resolve the caller's Stripe customer from the JWT
``sub`` claim, so the minted token's user must match ``TEST_CLERK_USER_ID``.
...
def mint_jwt(secret_key: str, user_id: str, template: str) -> str:
    ...
    Args:
        ...
        user_id: Clerk user the session/token is minted for (``user_...``). The
            billing endpoints map this ``sub`` to a Stripe customer.
