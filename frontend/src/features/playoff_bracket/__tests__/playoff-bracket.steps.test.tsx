...
defineFeature(feature, (test) => {
  ...
  test('A no-data 404 for the bracket query shows the predictor, not the raw error', ({
    given,
    when,
    then,
  }) => {
    given(
      'the latest season is in progress and the bracket query returns a no-data 404',
      () => {
        // Same in-progress shape as above, but PLAYOFF_BRACKET is left UNMAPPED so
        // the mock 404s it (the real backend's "no data" response) instead of
        // returning an empty array. getPlayoffBracket must tolerate that 404.
        const played = { team_a_score: 100, team_b_score: 90 };
        const unplayed = { team_a_score: 0, team_b_score: 0 };
        const base = {
          team_a_id: 't1',
        ...
        server.use(
          leagueQuery({
            // PLAYOFF_BRACKET intentionally omitted → mock responds 404.
            MATCHUPS: [
              { ...base, ...played, week: '1' },
              { ...base, ...unplayed, week: '2' },
            ],
