...
describe('computeScoreDistribution', () => {
  it('builds the five-number summary from a manager regular-season scores', () => {
    ...
    // No outliers (fences are [-10, 70]); whiskers reach the raw min/max.
    expect(t1.outliers).toEqual([]);
    expect(t1.whiskerLow).toBe(10);
    expect(t1.whiskerHigh).toBe(50);
  });

  it('flags Tukey-fence outliers and pulls whiskers in to the fence', () => {
    // T1 scores: 50, 90, 100, 110, 120, 200 → fences ≈ [55, 155].
    const matchups = [
      game('1', 'T1', 50, 'T2', 5),
      game('2', 'T1', 90, 'T2', 5),
      game('3', 'T1', 100, 'T2', 5),
    ...
    const { teams } = computeScoreDistribution(matchups);
    const t1 = teams.find((t) => t.teamId === 'T1')!;
    expect(t1.outliers).toEqual([50, 200]);
    expect(t1.whiskerLow).toBe(90);
    expect(t1.whiskerHigh).toBe(120);
  });
