...
testWithDemo.describe('Demo mode — protected pages', () => {
  ...
  testWithDemo(
    'no protected page redirects to landing page',
    async ({ page }) => {
...
testWithDemo.describe('Demo mode — page content', () => {
  testWithDemo(
    '/home renders section headings and stat labels',
    async ({ page }) => {
      await page.goto('/home');
      await page.waitForLoadState('networkidle');
      await expect(page.getByText('Champions', { exact: true })).toBeVisible();
      await expect(
        page.getByText('Final Standings Position by Season'),
      ).toBeVisible();
  ...
  testWithDemo(
    '/standings renders award cards, table headers, and chart',
    async ({ page }) => {
      await page.goto('/standings');
      await page.waitForLoadState('networkidle');
  ...
  testWithDemo(
    '/playoff_bracket renders bracket round headers',
    async ({ page }) => {
  ...
  testWithDemo(
    '/matchup_records renders all record type cards',
    async ({ page }) => {
  ...
  testWithDemo(
    'sidebar renders settings section with demo mode controls',
    async ({ page }) => {
      await page.goto('/home');
