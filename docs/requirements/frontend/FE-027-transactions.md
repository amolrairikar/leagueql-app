...
season: one row per owner with columns Waivers, Free Agents, Trades, and Total. Each owner row
shows the owner's avatar with their username and team name beneath it, consistent with the other
tables (e.g. Season Standings). The avatar logo and color are **reused from Season Standings**
([FE-005](FE-005-season-standings.md)) so an owner looks identical across pages: the page also
loads that season's `SEASON_STANDINGS` view and joins on roster id (transactions' `roster_id`
equals standings' `team_id` for Sleeper), taking the standings logo and the standings *order
index* (the standings color is positional — `avatarColor(index)`). When standings is missing or
fails to load, or a roster is absent from it, the row falls back to its own index-based color and
initials, so the summary never breaks. Counts are
...
- Summary avatar/color: the page also reads the season's `SEASON_STANDINGS` view via
  `getSeasonStandings` (`src/features/season_standings/api-calls.ts`) and joins on roster id to
  reuse the Season Standings ([FE-005](FE-005-season-standings.md)) logo + color; the load is
  fault-tolerant (a 404/error just disables the reuse).
...
- **Standings missing for the summary avatar:** if the season's standings view is absent, fails
  to load, or omits a roster, that owner's row falls back to an index-based color and initials
  (no logo) rather than failing — the summary still renders from transactions data alone.
...
- [ ] The summary table lists one row per participating owner — each with the owner's avatar,
      username, and team name — with per-transaction Waivers, Free Agents, Trades, and Total
      counts, ordered by Total descending; it renders nothing on an empty or failed load.
- [ ] Each summary row reuses the owner's Season Standings avatar logo and color (joined on
      roster id), falling back to an index-based color + initials when standings is unavailable.
