...
- **Checkout endpoint** (Clerk-authed): `POST /leagues/{id}/checkout-session` — takes a required
  ...
  optional `returnPath` query param — the in-app path the user started checkout from — used to
  build **both** the Checkout `success_url` and the `cancel_url` ("back" button) so that whether
  the user completes or cancels payment they return to that page rather than the dashboard home.
  The `success_url` carries `?checkout=success` (merged with any existing query) to drive the
  activation poll; the `cancel_url` has no param. To avoid an open redirect the path is honored
  only when it is a **safe same-origin relative path** (a single leading `/`, no protocol-relative
  `//`, no backslashes or whitespace); otherwise (or when absent) both URLs fall back to `…/home`
  (success still carrying `?checkout=success`).
  Customers redeem a
...
- **Webhook endpoint / Lambda** (no Clerk auth; `Stripe-Signature` verified): `POST
  ...
  no-op), the webhook also fires a fire-and-forget invoke of the recap-generator Lambda
  ([BE-022](BE-022-ai-weekly-matchup-recap.md)) to backfill the newly-premium league's weekly
  recaps; a failed invoke is swallowed (the webhook still returns 200). The webhook is now an
  **OTel-traced Lambda** ([BE-021](BE-021-async-chain-otel-propagation.md)): it calls
  ...
  W3C context, so it starts a **root** span), which parents the recap invoke so the activation path
  is one continuous trace.
...
- **Infrastructure (now implemented).** The `stripe_webhook` Lambda is deployed per-region
  ...
  `https://leagueql.com`, dev → `http://localhost:5173`). The Billing Portal **return** targets the
  in-app dashboard home (`…/home`, under the SubscriptionGuard). Checkout **success** and **cancel**
  both return to the page checkout was started from (the `returnPath` query param appended to the
  configured URL's origin), falling back to `…/home`. Success carries `?checkout=success` (merged
  with any existing query), which drives the activation poll in
