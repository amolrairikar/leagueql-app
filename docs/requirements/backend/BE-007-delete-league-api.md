...
resolves the canonical league ID, removes all DynamoDB items for the league, deletes the
raw API payloads from S3, and decrements the global `LEAGUE_COUNT`.
...
## Sources
`src/api/routes.py::delete_league`, `docs/api/openapi_spec.yaml`.
