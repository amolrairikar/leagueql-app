...
The API SHALL return the single matching item's `data` with `200` for a valid suffixed `queryType`.

#### Scenario: Suffixed query
- **WHEN** a valid suffixed `queryType` such as `STANDINGS#2024` is queried
