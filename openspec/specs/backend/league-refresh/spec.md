...
The API SHALL reject a refresh while one is already in progress and while within the cooldown window, and SHALL update `last_refresh_at` on success.
...
- **WHEN** `last_refresh_at` is within `REFRESH_COOLDOWN_MINUTES`
- **THEN** the API returns `429` with the remaining wait time

#### Scenario: Cooldown reset on success
- **WHEN** a refresh succeeds
- **THEN** `last_refresh_at` is updated to enforce the next cooldown window
