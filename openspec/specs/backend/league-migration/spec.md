...
## Purpose
Migrate an onboarded league from one platform to another (e.g. ESPN → Sleeper) while preserving all-time history under a single canonical league ID. `POST /leagues/{leagueId}/migrate` records the manager identity mapping in a `PLATFORM_MIGRATION` item, updates `METADATA` to the new active platform, then invokes the onboarder for the destination platform. The onboarder writes the destination `LEAGUE_LOOKUP` only after it successfully fetches the destination league, and reads the mapping to resolve cross-platform owner IDs so metrics stay continuous.
