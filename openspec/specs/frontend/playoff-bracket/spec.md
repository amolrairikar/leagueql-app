...
When the selected season has no bracket data, a clear empty-state message SHALL render instead of the blank round columns.

#### Scenario: No playoffs yet
- **WHEN** the `PLAYOFF_BRACKET#{season}` query succeeds but returns no matches
